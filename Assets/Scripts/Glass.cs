...
public class Glass : MonoBehaviour
{
    ...
    {
        if (transform.GetChild(0).GetComponent<Ball>().type != Ball.Type.clear)
        {
            for (int i = 1; i <= 3; i++)
            {
                if (transform.GetChild(i).GetComponent<Ball>().type == transform.GetChild(0).GetComponent<Ball>().type)
                {
                    if (transform.GetChild(3).GetComponent<Ball>().type == transform.GetChild(0).GetComponent<Ball>().type)
                    {
                        glassIsFull = true;
